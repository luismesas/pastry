	...
	self.parseCookies = function(p_options){
		...
		return function (req, res, next){
			...
			if(req.pastry.sid === undefined){
				...
				res.setHeader('Set-Cookie', createCookieString(config.prefix+'.sid',req.pastry.sid, _cookie));
			}
			...
			_dbClient.get(config.prefix+':session:' + req.pastry.sid, function(err, data){
				if(err){
					...
					if(data !== undefined){
						req.pastry.data = JSON.parse(data);
					}
				}
				next();
				reqPause.resume();
